...
//    write("h is "+(string)h+"\n");
//    file = _Server->get_module("filepath:tree")->path_to_object(argv[-1]);
//    applaunch(file,demo);
    import_from_git(options->src, options->dest);
    return 0;
}
/*
function edit_now(int argc,array(string) argv)
{
  object p = ((program)"edit.pike")();
  _Server=p->_Server;
  return p->main(argc,argv);
